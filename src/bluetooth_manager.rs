...
impl fmt::Display for BluetoothManagerError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            BluetoothManagerError::DisconnectionFailed(e) => {
                write!(f, "Disconnection failed: {}", e)
            }
...
impl BluetoothManager {
    ...
    pub async fn list_paired_devices(&self) -> Result<Vec<PairedDevice>, Box<dyn Error>> {
        ...
        for device in devices {
            ...
            let device_type = if device.is_le().await.unwrap_or(false)
                && device.is_bredr().await.unwrap_or(false)
            {
                DeviceType::DualMode
        ...
        for p in peripherals {
            if let Ok(Some(props)) = p.properties().await {
                let matches_name = props
                    ...
                    .unwrap_or(false);
                let matches_addr =
                    props.address.to_string().to_lowercase() == identifier.to_lowercase();
