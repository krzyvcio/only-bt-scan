...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Check if backup was created (corrupted database detected)
    if db::backup_exists() {
        execute!(
            stdout(),
        ...
        writeln!(
            ...
            "{}",
            format!(
                "⚠️  Corrupted database backed up to: {}",
                db::get_backup_path()
            )
            .yellow()
        )?;
    ...
    while !shutdown_in_progress.load(std::sync::atomic::Ordering::Relaxed) {
        ...
        match scanner.run_scan().await {
            Ok(devices) => {
                // Send Telegram notification for new devices BEFORE saving to database
                // This way is_device_new() can correctly detect new devices
                if telegram_notifier::is_enabled() {
